...
- **Khắc phục lỗi mất hiển thị nền Pet Slot theo độ hiếm (Chuyển sang Lua table)**:
  - **Lưu cấu hình PetConfig trong code**: Khai báo bảng ảnh nền `PetClass.RARITY_BG` trực tiếp trong tệp [PetClass.luau](file:///d:/Study/Unity/VNG/project/src/shared/PetClass.luau). Việc này ngăn chặn Rojo tự động xóa thư mục cấu hình khi đồng bộ hóa từ ổ đĩa sang Roblox Studio. Xóa bỏ thư mục `src/shared/PetConfig` và các file cấu hình `.txt` dư thừa.
  - **Tối ưu hóa tải ảnh nền**: Cập nhật hàm cập nhật giao diện trong [UIController.client.luau](file:///d:/Study/Unity/VNG/project/src/client/UIController.client.luau) để đọc trực tiếp từ `PetClass.RARITY_BG[rarity]` thay thế cho việc tìm kiếm `StringValue` cũ dưới `ReplicatedStorage`, giúp tăng tốc độ xử lý và triệt tiêu lỗi bất đồng bộ.
...
- **Tích hợp Hệ thống Bảo hiểm Gacha (Pity System)**:
  - **Lưu trữ Pity trên DataStore**: Cập nhật tệp [DataManager.server.luau](file:///d:/Study/Unity/VNG/project/src/server/DataManager.server.luau) để tích hợp 2 thuộc tính `RarePityCount` và `LegendaryPityCount` lưu trực tiếp xuống Cloud DataStore và nạp lên RAM Cache của Server, đồng thời expose các hàm tăng/reset bộ đếm pity.
  - **Thuật toán quay Gacha Bảo hiểm**: Nâng cấp thuật toán `RollPet(player)` trong tệp [GachaModule.luau](file:///d:/Study/Unity/VNG/project/src/shared/GachaModule.luau). Áp dụng quy tắc bảo hiểm cứng (Hard Pity) trúng 100% Rare sau 8 lần trượt hoặc Legendary sau 15 lần trượt, kết hợp tăng tỷ lệ bảo hiểm mềm (Soft Pity) tại mốc 5 lần trượt Rare (lên 35%) và 10 lần trượt Legendary (lên 25%).
  - **Đồng bộ Client UI Dynamic RichText**: Cập nhật tệp [UIController.client.luau](file:///d:/Study/Unity/VNG/project/src/client/UIController.client.luau) để tính toán tỷ lệ động theo số lần hụt Pity. Sử dụng thuộc tính `RichText = true` và nhuộm màu chữ bằng thẻ font màu (vàng rực `#FFD700` cho Rare và đỏ cam `#FF5555` cho Legendary) khi tỷ lệ được gia tăng, đồng bộ lập tức sau mỗi lần quay và tự động phục hồi đúng trạng thái khi người chơi mới vào game.
