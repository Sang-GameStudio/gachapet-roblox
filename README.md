...
| **9. Phân loại khung nền Slot (Slot UI Rarity BG)** | Ảnh nền các ô khung Slot UI trong túi đồ tự động đổi màu sắc viền/nền tương ứng theo từng độ hiếm (Common: cát nhạt, Rare: tím, Legendary: vàng) giúp phân cấp vật phẩm trực quan. | 📁 `src/client/UIController.client.luau`<br/>↳ Hàm `refreshInventoryUI()` (gán ảnh từ `PetConfig.RARITY_BG`) |
| **10. Thuật toán sắp xếp túi đồ (Inventory Sorting)** | Tự động sắp xếp (Sort) các ô Pet trong túi đồ theo thứ tự ưu tiên độ hiếm tăng dần (Common -> Rare -> Legendary). Nếu cùng độ hiếm sẽ xếp theo bảng chữ cái để thuận tiện cho việc quản lý hòm đồ. | 📁 `src/client/UIController.client.luau`<br/>↳ Hàm `refreshInventoryUI()` (sử dụng thuật toán `table.sort`) |
